...
yAux = ones(m, num_labels);
for i = 1:m
	yAux(i,:) = eye(num_labels)(y(i),:);
end
...
for i = 1:m
	for k = 1:num_labels
		...
		J = J + -yAux(i,k) * log( a3(i,k) ) - (1 - yAux(i,k)) * log( 1 - a3(i,k) );
		
	end
end	

Regularization = sum(sum(Theta1 .^ 2)) - sum(Theta1(:,1) .^ 2) + sum(sum(Theta2 .^ 2)) - sum(Theta2(:,1) .^ 2);
